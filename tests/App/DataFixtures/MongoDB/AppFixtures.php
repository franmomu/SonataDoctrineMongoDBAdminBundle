...
final class AppFixtures extends Fixture
{
    public function load(ObjectManager $manager): void
    {
        ...
        $miguel = new Author('author_id', 'Miguel de Cervantes');
        $miguel->setAddress(new Address('Somewhere in La Mancha, in a place whose name I do not care to remember'));
        $miguel->addPhoneNumber(new PhoneNumber('666-666-666'));
        $book = new Book('book_id', 'Don Quixote', $miguel);
        $book->addCategory($novelCategory);

        $manager->persist($miguel);
